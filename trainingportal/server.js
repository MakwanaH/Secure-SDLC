...
var key = fs.readFileSync(path.join(__dirname, 'selfsigned.key'));
var cert = fs.readFileSync(path.join(__dirname, 'selfsigned.crt'));
var options = {
    key: key,
    cert: cert
};
...
const apiLimiter = rateLimit({
    ...
    onLimitReached: (request, response) => {
        util.log(`API rate limit reached: IP -  '${request.ip}', endpoint: '${request.path}`)
    },
})
...
const generalLimiter = rateLimit({
    ...
    legacyHeaders: true,
    onLimitReached: (request, response) => {
        util.log(`General limit reached: IP -  '${request.ip}', endpoint: '${request.path}`)
    },
})
...
const solutionHiding = require(path.join(__dirname, 'solutionHiding'));
...
var configureSolutionsHtml = fs.readFileSync(path.join(__dirname, 'static/configure-solution.html'), "utf8");
...
app.use('/public/jquery/dist/jquery.min.js',express.static(path.join(__dirname, 'node_modules/jquery/dist/jquery.min.js')));
app.use('/public/angular/angular.min.js',express.static(path.join(__dirname, 'node_modules/angular/angular.min.js')));
app.use('/public/angular-route/angular-route.min.js',express.static(path.join(__dirname, 'node_modules/angular-route/angular-route.min.js')));
...
app.use('/public/bootstrap/dist/js/bootstrap.min.js',express.static(path.join(__dirname, 'node_modules/bootstrap/dist/js/bootstrap.min.js')));
app.use('/public/bootstrap/dist/css/bootstrap.min.css',express.static(path.join(__dirname, 'node_modules/bootstrap/dist/css/bootstrap.min.css')));
app.use('/public/open-iconic/font/css/open-iconic-bootstrap.min.css',express.static(path.join(__dirname, 'node_modules/open-iconic/font/css/open-iconic-bootstrap.min.css')));
app.use('/public/open-iconic/font/fonts',express.static(path.join(__dirname, 'node_modules/open-iconic/font/fonts')));
app.use('/public/highlightjs/styles/darkula.css',express.static(path.join(__dirname, 'node_modules/highlightjs/styles/darkula.css')));
app.use('public/canvas-confetti/dist/confetti.browser.js',express.static(path.join(__dirname, 'node_modules/canvas-confetti/dist/confetti.browser.js')));
...
app.get("/public/providers",(req,res) => {
  ...
  if("adminUsersPath" in config) providers.push({"name": "Admin", "url": "/public/provider/admin"})
...
app.get('/public/provider/:provider', (req,res) => {
  ...
  else if(req.params.provider == "admin") redirect = '/public/adminlogin.html';
...
app.post('/public/adminlogin', [
      auth.checkCaptchaOnLogin,
      passport.authenticate('admin', { failureRedirect: '/public/authFail.html' })
    ],
    function(req, res) {
      res.redirect('/main');
    });
...
app.get("/public/badge/:code",async(req,res) => {
  ...
  let imgSrc = `/public/badge/${encodedCode}/image.png`
  let html = badgeHtml;
  html = html.replace(/BADGE_IMG_SRC/g, imgSrc);
  html = html.replace("BADGE_URL", `${config.dojoUrl}/public/badge/${encodedCode}`);
  res.send(html);
});
...
app.get('/configure-solutions', (req, res) => {
    if (!auth.verifyAdmin(req.user)) {
        return util.apiResponse(req, res, 403, "You are not allowed here");
    }
    res.send(configureSolutionsHtml);
});

app.post("/api/challenges/configure-solution", (req, res) => {
    if (!auth.verifyAdmin(req.user)) {
        return util.apiResponse(req, res, 403, "You are not allowed here");
    }
    let challengeId = req.body.change.challengeId.trim();
    let hide = req.body.change.hide;
    if(!(hide === true) && !(hide === false)){
        return util.apiResponse(req, res, 400, "hide boolean param is mandatory");
    }
    if(util.isNullOrUndefined(challengeId) || util.isAlphanumericOrUnderscore(challengeId) === false) {
        return util.apiResponse(req, res, 400, "Invalid challenge id");
    }
    if(!challenges.checkChallengeIdExistence(challengeId)){
        return util.apiResponse(req, res, 400, "Challenge does not exist");
    }
    if(!challengeId.startsWith('cwe')){
        return util.apiResponse(req, res, 400, "Only challenges with CWE can be modified ")
    }
    solutionHiding.configure_challenge(challengeId, hide);
    return util.apiResponse(req, res, 200, "Done");
})
