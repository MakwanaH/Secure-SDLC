...
loadModules = function(){ 
    ...
    let localModules = {};
    moduleIds = Object.keys(moduleDefs);
...
function getDefinitionsForModule(moduleId){
    ...
    let p = path.join(__dirname, getModulePath(moduleId), '/definitions.json');
    let pp = path.resolve(p);
    if (!pp.startsWith(__dirname)) {
        util.log("Trying to access files outside of " + __dirname + ", " + pp);
        return null;
    }
    var defs = Object.freeze(require(pp));
    return defs;
}
...
exports.getSolution = function (challengeId) {
    if (solutionHiding.is_challenge_hidden(challengeId)) {
        return "Solution is hidden";
    }
...
module.exports.checkChallengeIdExistence = function(challengeId) {
    if(util.isNullOrUndefined(challengeId)) {
        return false;
    }
    return challengeId in challengeNames;
}