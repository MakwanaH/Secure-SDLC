...
try{
    if(!util.isNullOrUndefined(config.adminUsersPath))
    {
        let dataDir = util.getDataDir();
        adminUsersPath = path.join(dataDir, config.adminUsersPath);

        if(!fs.existsSync(adminUsersPath)){
            //create the users file if not already there
            fs.writeFileSync(adminUsersPath, "{}", 'utf8');
        }
        adminUsers = require(adminUsersPath);
    }
}
catch(ex){
    util.log(ex);
...
let registerAdminUser = function(){
    if(adminUsers == null){
        util.log("Admin users is not initialized")
        adminUsers = {}
    }
    let username = process.env.ADMIN_USERNAME
    let password = process.env.ADMIN_PASSWORD
    if (util.isNullOrUndefined(password)) {
        util.log("Set admin password into ADMIN_PASSWORD env var")
        return;
    }
    if (util.isNullOrUndefined(username)) {
        util.log("Set admin username into ADMIN_PASSWORD env var")
        return;
    }
    let admin = {"givenName":"Admin", "familyName":"Admin"};
    if (username in adminUsers) {
        util.log("Admin is already registered");
        return;
    }
    createUpdateAdmin(username, admin, password);
};

let verifyAdmin = function(user) {
    let username = user.accountId;
    if (util.isNullOrUndefined(username)) {
        return false;
    }
    ...
    return username in adminUsers;
}

let createUpdateAdmin = function(username, adminUser, password) {
    adminUser.passSalt = crypto.randomBytes(16).toString('base64').toString();
    adminUser.passHash = util.hashPassword(password,adminUser.passSalt);
    adminUsers[username] = adminUser;
    //save to disk
    let json = JSON.stringify(adminUsers, null, "\t");
    fs.writeFileSync(adminUsersPath, json, 'utf8');
}
...
let verifyAdminUserPassword = function(username, password) {
    if (adminUsers === null) {
        util.log("Admin authentication is configured");
        return null;
    }
    if (username in adminUsers) {
        let user = adminUsers[username];
        let saltString = user.passSalt;

        let passwordHash = util.hashPassword(password, saltString);
        if ( user.passHash === passwordHash) {
            return user;
        }
        else {
            util.log("Authentication failure for admin:" + username);
        }
    }
    else {
        util.log("Admin '"+username+"' not found.");
    }
    return null;
}
...
let getAdminStrategy = function () {
    return new LocalStrategy((username, password, cb) => {
        var user = verifyAdminUserPassword(username, password)
        if(user!=null) {
            return processAuthCallback("Admin_"+username, user.givenName, user.familyName, null, cb)
        }
        return cb(null,false);
    })
}
...
let getPassport = function (){
    ...
    if ("adminUsersPath" in config) passport.use("admin", getAdminStrategy());
